    ...
    const fetchSettings = async () => {
      try {
        const response = await fetch(
          `/api/settings?clientFolder=${encodeURIComponent(
            clientFolder,
  ...
  return (
    <form className={styles.settingsForm} onSubmit={handleSubmit}>
      <label className={styles.outLable}>
        <div className={styles.innerLable}>
          {' '}
          System <br /> Prompt:{' '}
        </div>
        <textarea
      ...
      <label className={styles.outLable}>
        ...
        <div style={{ marginLeft: 20, fontWeight: 700 }}>
          ...
          {chatTemperature < 0.5
            ? ' Reasoning '
            : chatTemperature > 0.5 && chatTemperature < 0.7
            ? 'Medium'
            : ' Creative '}
        </div>
      </label>
      ...
      <label className={styles.outLable}>
        ...
        <select
          ...
          onChange={(e) => setAllowChatHistory(e.target.value)}>
