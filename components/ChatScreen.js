  ...
  const fetchUrls = async () => {
    try {
      const response = await axios.get(
        `/api/urls/${clientName}/${botName}?file=settings.json`,
      );
  ...
  return (
    <>
      ...
      <main
        ...
        style={{
          textAlign: 'left',
          backgroundColor: settings.mainBG || '#f9f9f9',
          fontFamily: settings.mainFont || 'Arial',
          borderRadius: preview ? 20 : 0,
          height: preview ? 'auto' : '100vh',
        }}>
        <div
          className={styles.cloud}
          style={{
            width: preview ? '100%' : '85vw',
            height: preview ? 500 : 'initial',
          }}>
          <div
            ref={messageListRef}
            className={styles.messagelist}
            style={{ height: preview ? '100%' : '80vh' }}>
            <div
              style={{
                position: 'relative',
                backgroundColor: settings.mainBG || '#f9f9f9',
              ...
              if (message.type === 'apiMessage') {
                ...
                icon = (
                  <Image
                    src={
                      settings.userIcon ||
                      'https://cdn.jawwy.tv/9/avatar-smile.svg'
                    }
                    alt='Me'
                    width='35'
                    height='35'
                    ...
                    style={{
                      backgroundColor: settings.userIconColor || '#f9f9f9',
                    }}
                    priority
                  />
                );
                  ...
                  style={{
                    color:
                      message.type !== 'apiMessage'
                        ...
                        : settings.systemMessageColor,
                    backgroundColor:
                      message.type !== 'apiMessage'
                        ? settings.userMessageBG
                        : settings.systemMessageBG,
                    maxWidth: preview ? '90%' : '80%',
                    fontSize: preview ? '15px' : '16px',
                  }}>
                  {icon}
                  <div
                    ...
                    <ReactMarkdown
                      ...
                      transformLinkUri={(uri) => {
                        return uri.startsWith('https://deeplink')
                          ? `stctv://${uri}`.replace('https://', '')
                          : uri;
                      }}>
                      {message.message}
                    </ReactMarkdown>
        ...
        <div
          ...
          style={{ width: preview ? '140%' : 'auto' }}>
          <div
            className={styles.cloudform}
            style={{ width: preview ? '70%' : '100%' }}>
            <form onSubmit={handleSubmit}>
              <textarea
                ...
                style={{
                  backgroundColor: settings.promptBG || '#F1F1F1',
                  color: settings.promptColor || '#000000',
                  fontFamily: settings.mainFont || 'Arial',
                  width: preview ? '100%' : 'calc(100vw - 100px)',
                }}
              />
              <button
                type='submit'
                ...
                style={{ backgroundColor: settings.submitBG || '#0084FF' }}>
                {loading ? (
